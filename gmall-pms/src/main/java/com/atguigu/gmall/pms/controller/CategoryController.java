...
public class CategoryController {
    ...
    public Resp<List<CategoryVo>> querysubCates(@PathVariable("pid") Long pid){
        List<CategoryVo> CategoryVos=categoryService.querysubCates(pid);
        return Resp.ok(CategoryVos);
    }

    @GetMapping
    public Resp<List<CategoryEntity>> queryAllCategoryTree(@RequestParam(value = "level",defaultValue = "0") Integer level,
                                                           @RequestParam(value = "parentCid",required = false) Long pid){
        List<CategoryEntity> categoryEntities=categoryService.queryAllCategoryTree(level,pid);
