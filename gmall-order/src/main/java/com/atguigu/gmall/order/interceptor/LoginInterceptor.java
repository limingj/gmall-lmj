...
public class LoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if(StringUtils.isEmpty(userKey)){
			...
			CookieUtils.setCookie(request,response,this.jwtConfig.getUserKey(),userKey,this.jwtConfig.getExpireTime());
		}
		...
		//不为空  解析token
		try {
			Map<String, Object> infoToken = JwtUtils.getInfoFromToken(token, this.jwtConfig.getPublicKey());
